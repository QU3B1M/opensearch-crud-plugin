/*
 * SPDX-License-Identifier: Apache-2.0
 *
 * The OpenSearch Contributors require contributions made to
 * this file be licensed under the Apache-2.0 license or a
 * compatible open source license.
 */
package org.opensearch.consumer.action;

import org.opensearch.action.update.UpdateRequest;
import org.opensearch.common.xcontent.XContentFactory;
import org.opensearch.core.xcontent.ToXContent;
import org.opensearch.core.xcontent.XContentParser;
import org.opensearch.rest.RestRequest;
import org.opensearch.consumer.model.Consumer;

import java.io.IOException;

import static org.opensearch.core.xcontent.XContentParserUtils.ensureExpectedToken;

/**
 * Handles the updating of consumer in OpenSearch.
 */
public class RestConsumerUpdateAction {

    /**
     * Creates an UpdateRequest for updating a consumer.
     *
     * @param request The RestRequest containing the consumer details.
     * @return The UpdateRequest to update the consumer.
     * @throws IOException If an I/O error occurs.
     */
    public static UpdateRequest updateRequest(RestRequest request) throws IOException {
        String consumerId = request.param("id");
        if (consumerId == null) {
            throw new IllegalArgumentException("Missing required parameters: id is mandatory");
        }

        XContentParser parser = request.contentParser();
        ensureExpectedToken(XContentParser.Token.START_OBJECT, parser.nextToken(), parser);
        Consumer consumer = Consumer.fromXContent(parser);

        UpdateRequest updateRequest = new UpdateRequest(Consumer.TASK_INDEX, consumerId);
        updateRequest.doc(consumer.toXContent(XContentFactory.jsonBuilder(), ToXContent.EMPTY_PARAMS));
        updateRequest.fetchSource(true);

        return updateRequest;
    }
}
